...
#include <string.h>
float salarioMinimo = 0.0, valorBanho = 0.0, valorBanhoTosa = 0.0, descontoBanhoTosa = 0.0, valorTotal = 0.0;
int opcaoEscolha, qtdBanho, qtdBanhoTosa;
...
//Procedure
void solicitarSalaioMinimo() {
	printf ("Por favor, informe o salário mínimo: ");
	scanf ("%f", &salarioMinimo);
}

void menuPetshop () {
	printf ("Escolha (1) para banho, (2) para banho e tosa e (3) para ambos: ");
	scanf ("%i", &opcaoEscolha);
}


// Function
float calcularDescontoBanhoTosa () {
	float descontoBanhoTosa = valorBanhoTosa * 0.07;
	return descontoBanhoTosa;
}

float calcularValorBanho() {
	valorBanho = valorBanhoTosa + descontoBanhoTosa / 2;
	return valorBanho;
}

float calcularValorBanhoTosa() {
	valorBanhoTosa = salarioMinimo * 0.055;
	descontoBanhoTosa = calcularDescontoBanhoTosa();
	valorBanhoTosa = valorBanhoTosa - descontoBanhoTosa;
	return valorBanhoTosa;
}

int informarQuantidadeBanho () {
	printf ("Informe a quantidade de banho: ");
	scanf ("%i", &qtdBanho);
	return qtdBanho;
}

int informarQuantidadeBanhoTosa () {
	printf ("Informe a quantidade de banho e tosa: ");
	scanf ("%i", &qtdBanhoTosa);
	return qtdBanhoTosa;	
}

float calcularTotal () {
	valorTotal = (valorBanho * qtdBanho) + (valorBanhoTosa * qtdBanhoTosa);
}

main() {
    setlocale(LC_ALL, "Portuguese_Brazil.1252");
   	system ("chcp 1252 > nul"); // Configurar o console para UTF-8
   	system ("title Petshop do Epaminondas");
	...
	// Processing
		solicitarSalaioMinimo();
		calcularValorBanhoTosa();
		calcularValorBanho();
		calcularDescontoBanhoTosa();
		
		menuPetshop();
		
		if (opcaoEscolha == 1) {
			informarQuantidadeBanho();
			calcularTotal();
		} 
		else if (opcaoEscolha == 2) {
			informarQuantidadeBanhoTosa();
			calcularTotal();
		} 
